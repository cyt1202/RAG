...
这个命令会处理 `{base_filename}.pdf`，为它构建新的知识图谱和向量库（保存在 `output/{base_filename}_KG.json` 和 `vector_db/{base_filename}_faiss_index`），然后回答指定问题，并将结果保存。
...
本项目的核心是构建一个能够深度理解复杂文档并提供精准回答的RAG系统。为了实现这一目标，我没有采用单一的检索策略，而是设计了一套结合了先进语义分割和知识图谱的 **混合检索（Hybrid Retrieval）** 架构。
...
为了应对这些挑战，我对流程中的两个关键环节——**文档切割**和**信息检索**——进行了深度优化。

### 1. 语义分割策略的演进

我对三种不同层次的文本分割方法进行了评估，旨在找到保留语义最完整、最适合下游检索的方案。
...
* **模型驱动方法：`chonky` (本项目最终选用)**
    ...
    * **结论**：在我的测试中，`chonky` 的分割结果在语义完整性上表现最佳。它能更好地理解句子和段落间的内在联系，生成高质量的文本块，为后续的向量检索奠定了坚实的基础。
...
> **理解：** 知识图谱与向量数据库并非竞争关系，而是**互补关系**。向量数据库能快速找到相关段落；而知识图谱则能提供整个知识体系的结构化骨架。
